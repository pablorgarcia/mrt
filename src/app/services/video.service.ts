...
export class VideoService {
  ...
  private myVideos: any | undefined;
  ...
  public getYoutubeList(load : boolean = true, collection: string = 'hotLapId'): Observable<any[]> {
    return new Observable((subs) => {
      if(load) {
        const apiKeys: any = environment.YOUTUBE_CONF.videoList;
        const myVideos: any = {};
        Object.keys(apiKeys).forEach((k: string) => {
          this.http.get(VideoService.urlYoutubeList(apiKeys[k])).subscribe((data: any) => {
            myVideos[k] = data.items;
            if(this.myVideos) {
              collection === k ?
                this.myVideos[k] = [...this.myVideos[k], ...myVideos[k]] : this.myVideos[k] = myVideos[k]
              ;
            }
            if(Object.keys(myVideos).length === Object.keys(apiKeys).length) {
              if(!this.myVideos) { this.myVideos = myVideos; }
              subs.next(this.myVideos);
            }
          })
        });
      } else {
